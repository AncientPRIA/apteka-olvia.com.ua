...
self.addEventListener('fetch', function(event) {
	...
	for(let i = 0;i<exclude_url.length;i++){
		if ( event.request.url.match( '^.*('+exclude_url[i]+').*$' ) ) {
			console.log("!!!! page not work !!!!");
			exclude_flag = true;
			break;
			//return false;
		}
	}
	// if ( event.request.url.match( '^.*(\/admin\/).*$' ) || event.request.url.match( '^.*(\/admin).*$' ) ) {
	// 	console.log("!!!! page not work !!!!");
	// 	return false;
	// } else{
	if(exclude_flag !== true){
		event.respondWith(async function() {
			for(let i=0;i<exclude_content.length;i++){
				if(event.request.url.match( '^.*('+exclude_content[i]+').*$' ) === null){
